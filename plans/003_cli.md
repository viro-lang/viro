...
#### T1.1: Exit Codes and Error Mapping (Day 1)
...
#### T1.2: Configuration System (Days 1-2)
...
func main() {
    ...
    mode, err := detectMode(cfg, flag.Args())
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error: %v\n", err)
        os.Exit(ExitUsage)
    }
...
#### T2.1: Script File Loading (Day 1)
...
#### T2.2: Script Execution (Days 2-3)
...
#### T2.4: Stdin Script Execution (Day 5)
...
#### T3.2: Stdin Integration (Days 3-4)
...
### Phase 4: Enhanced REPL Configuration (Week 4) - OPTIONAL
...
#### T4.1: REPL Options Structure (Days 1-2)
...
#### T4.2: Trace/Debug Startup Modes (Days 3-4)
...
**Implementation**: Enable trace/debug before starting REPL
...
#### T4.3: Color Output (Day 5)

**Priority**: LOW - Polish

**Files to modify**:
- `cmd/viro/config.go` - Add `--color` flag

**Implementation**: Add `--color` flag with auto/always/never options

**Success criteria**: Colors work and respect TTY detection
...
**Tests**: Test argument parsing and access from scripts

**Estimated**: 1 day

---

#### T5.2: Profiling Support
...
### Phase 6: Documentation and Polish (Week 6) - OPTIONAL
...
#### T6.2: Man Page and CLI Cookbook
...
4. **Week 6 Complete**: Production-ready with polish and docs
