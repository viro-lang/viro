...
- Contract coverage for data natives lives in `test/contract/data_test.go:1-370`, using table-driven tests plus the `Evaluate` helper; this file is the right place for `TestData_NoneQ`.
...
2. **Contract tests (TDD)**
   - Extend `test/contract/data_test.go` with `TestData_NoneQ`, following the existing table-driven style.
...
5. **Verification**
   - Run focused tests: `go test ./test/contract -run TestData_NoneQ` to iterate quickly.
...
## Integration Points
- **Spec ↔ tests**: The contract update should enumerate the same scenarios covered by `TestData_NoneQ` to keep QA tooling aligned.
...
- **Arity enforcement**: Predicate must gracefully error when missing arguments; rely on `arityError` to keep messaging consistent. Additional values after the function call are treated as separate expressions and do not trigger arity errors.
