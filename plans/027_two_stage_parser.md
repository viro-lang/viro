...
Viro currently uses a PEG (Parsing Expression Grammar) parser generated by Pigeon. The grammar mixes lexical analysis (tokenization) with semantic analysis (value construction), making it:

1. **Hard to extend**: Parser logic is embedded in grammar rules
2. **Not exposable**: Cannot provide parsing functions to Viro code for metaprogramming
3. **Complex**: PEG combines concerns that should be separated
4. **Dependent**: Requires Pigeon tool and generated code maintenance

Viro's syntax is simple enough for a two-stage parser:
...
- Metaprogramming: Expose `tokenize`, `parse`, `load` functions to Viro
- Simplicity: Each stage has single responsibility
- Maintainability: Plain Go code instead of grammar DSL
- Extensibility: Support custom dialects and user-defined syntax

Reference: `docs/two-stage-parser.md` for complete architecture description.

## Goals

1. Replace PEG parser with two-stage tokenizer + parser
2. Maintain 100% compatibility with existing behavior
3. Improve or maintain parsing performance
4. Expose parsing primitives to Viro code for metaprogramming
5. Remove Pigeon dependency

## Non-Goals

1. Change Viro syntax or semantics
2. Optimize parser beyond current performance
3. Add new language features (except exposing parser functions)
...
**Step 3.3: Remove PEG dependencies**

1. Delete `grammar/viro.peg`
2. Delete `internal/parse/peg/` directory
3. Remove Pigeon from tools
4. Update Makefile to remove `grammar` target
5. Update `make build` to not depend on grammar generation
...
Add section on two-stage parser:

- Link to `docs/two-stage-parser.md`
- Update build instructions (no grammar generation)
- Document new native functions

**Step 5.2: Update README.md**

Remove references to Pigeon/PEG parser.

**Step 5.3: Write migration notes**

Document any behavioral differences (should be none).

**Step 5.4: Performance benchmark**

Create benchmark comparing old PEG vs new two-stage:

```go
func BenchmarkParsePEG(b *testing.B) { /* ... */ }
func BenchmarkParseTwoStage(b *testing.B) { /* ... */ }
```

Expected: New parser is 10-30% faster.
...
Before removing PEG code:

1. Capture outputs of PEG parser for 100+ representative inputs
2. Verify new parser produces identical outputs
3. Document any intentional behavioral changes
