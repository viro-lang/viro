...
For positive numbers, behaves like floor; for negative numbers, behaves like ceil.`,
	...
	registerSimpleMathFunc("first", First, 1, &NativeDoc{
		Category: "Series",
...
For strings, returns the first character as a string.`,
	...
	registerSimpleMathFunc("last", Last, 1, &NativeDoc{
		Category: "Series",
...
For strings, returns the last character as a string.`,
	...
	registerSimpleMathFunc("append", Append, 2, &NativeDoc{
		Category: "Series",
...
Returns the modified series. For strings, the value is converted to a string before appending.`,
	...
	registerSimpleMathFunc("insert", Insert, 2, &NativeDoc{
		Category: "Series",
...
Returns the modified series. For strings, the value is converted to a string before inserting.`,
	...
	registerSimpleMathFunc("length?", LengthQ, 1, &NativeDoc{
		Category: "Series",
...
The word is not evaluated. Raises an error if the word is not bound to a value.`,
	...
	registerSimpleMathFunc("type?", TypeQ, 1, &NativeDoc{
		Category: "Data",
...
Blocks are formatted with spaces between elements. Returns none.`,
	...
	registerSimpleMathFunc("input", Input, 0, &NativeDoc{
		Category: "I/O",
...
The trailing newline is removed. Blocks until input is received.`,
	...
	registerSimpleMathFunc("open", OpenNative, 1, &NativeDoc{
		Category: "Ports",
...
File operations are subject to sandbox restrictions if configured.`,
	...
	registerSimpleMathFunc("close", CloseNative, 1, &NativeDoc{
		Category: "Ports",
...
After closing, the port should not be used for further I/O operations. Returns none.`,
	...
	registerSimpleMathFunc("read", ReadNative, 1, &NativeDoc{
		Category: "Ports",
...
opens the file, reads its contents, and closes it automatically. Returns the data as a string.`,
	...
	registerSimpleMathFunc("write", WriteNative, 2, &NativeDoc{
		Category: "Ports",
...
opens the file, writes the data, and closes it automatically. Overwrites existing content.`,
	...
	registerSimpleMathFunc("save", SaveNative, 2, &NativeDoc{
		Category: "Ports",
...
This is the recommended way to persist viro data structures.`,
	...
	registerSimpleMathFunc("load", LoadNative, 1, &NativeDoc{
		Category: "Ports",
...
Returns the parsed viro value (block, object, etc.).`,
	...
	registerSimpleMathFunc("query", QueryNative, 1, &NativeDoc{
		Category: "Ports",
...
The exact information returned depends on the port type. Returns an object with metadata fields.`,
	...
	registerSimpleMathFunc("wait", WaitNative, 1, &NativeDoc{
		Category: "Ports",
...
or none if the condition is initially false. Be careful to avoid infinite loops.`,
	...
	Registry["reduce"] = value.NewNativeFunction(
		"reduce",
		[]value.ParamSpec{
		...
		func(args []value.Value, refValues map[string]value.Value, eval value.Evaluator) (value.Value, error) {
			reverseAdapter := &nativeEvaluatorAdapter{eval}
			result, err := Reduce(args, refValues, reverseAdapter.unwrap())
			if err == nil {
				return result, nil
			}
			return result, err
	...
	fn.Doc = &NativeDoc{
		Category: "Control",
		Summary:  "Evaluates each element in a block and returns a new block with the results",
		Description: `Takes a block and evaluates each element within it, returning a new block containing
the evaluated results. This is useful for creating blocks with computed values, string interpolation,
and building data structures dynamically. Similar to REBOL's reduce function.`,
		Parameters: []ParamDoc{
			{Name: "block", Type: "block!", Description: "The block whose elements should be evaluated", Optional: false},
		},
		Returns:  "[block!] A new block with evaluated elements",
		Examples: []string{"x: 5\nreduce [1 + 1 x * 2]  ; => [2 10]", "name: \"Alice\"\nreduce [\"Hello\" name]  ; => [\"Hello\" \"Alice\"]", "reduce [2 + 3 4 * 5]  ; => [5 20]"},
		SeeAlso:  []string{"do", "compose"}, Tags: []string{"control", "block", "evaluation"},
	}
