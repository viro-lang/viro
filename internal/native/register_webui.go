...
func RegisterWebUINatives(rootFrame core.Frame, manager webui.Manager) {
	...
	webuiObj.Frame.Bind("start", value.NewFuncVal(value.NewNativeFunction(
		...
		func(args []core.Value, refValues map[string]core.Value, eval core.Evaluator) (core.Value, error) {
			return WebUIStart(args, refValues, eval, manager)
		},
		false,
		&NativeDoc{
			Category: "WebUI",
			Summary:  "Initialize the WebUI subsystem",
			Description: `Initializes the WebUI subsystem and returns default configuration.
This is called automatically by other webui.* functions.`,
			Returns:  "[object!] Default WebUI configuration",
			Examples: []string{"webui.start  ; => object with defaults"},
	...
	webuiObj.Frame.Bind("window", value.NewFuncVal(value.NewNativeFunction(
		...
		func(args []core.Value, refValues map[string]core.Value, eval core.Evaluator) (core.Value, error) {
			return WebUIWindow(args, refValues, eval, manager)
		},
		false,
		&NativeDoc{
			Category: "WebUI",
...
Returns a webui-window! value that can be used with other webui functions.`,
	...
	webuiObj.Frame.Bind("render", value.NewFuncVal(value.NewNativeFunction(
		...
		func(args []core.Value, refValues map[string]core.Value, eval core.Evaluator) (core.Value, error) {
			return WebUIRender(args, refValues, eval, manager)
		},
		false,
		&NativeDoc{
			Category: "WebUI",
...
Supports string!, binary!, and file! markup with optional content-type validation.`,
	...
	webuiObj.Frame.Bind("inject", value.NewFuncVal(value.NewNativeFunction(
		...
		func(args []core.Value, refValues map[string]core.Value, eval core.Evaluator) (core.Value, error) {
			return WebUIInject(args, refValues, eval, manager)
		},
		false,
		&NativeDoc{
			Category:    "WebUI",
	...
	webuiObj.Frame.Bind("send", value.NewFuncVal(value.NewNativeFunction(
		...
		func(args []core.Value, refValues map[string]core.Value, eval core.Evaluator) (core.Value, error) {
			return WebUISend(args, refValues, eval, manager)
		},
		false,
		&NativeDoc{
			Category: "WebUI",
...
Payload is JSON-encoded automatically.`,
	...
	webuiObj.Frame.Bind("on", value.NewFuncVal(value.NewNativeFunction(
		...
		func(args []core.Value, refValues map[string]core.Value, eval core.Evaluator) (core.Value, error) {
			return WebUIOn(args, refValues, eval, manager)
		},
		false,
		&NativeDoc{
			Category: "WebUI",
...
Handler block receives event-name, event-selector, event-payload, and event-window bindings.`,
	...
	webuiObj.Frame.Bind("poll", value.NewFuncVal(value.NewNativeFunction(
		...
		func(args []core.Value, refValues map[string]core.Value, eval core.Evaluator) (core.Value, error) {
			return WebUIPoll(args, refValues, eval, manager)
		},
		false,
		&NativeDoc{
			Category: "WebUI",
			Summary:  "Process pending events",
			Description: `Drains and processes pending events for the window.
Returns none when done or when manager is closed.`,
			Parameters: []ParamDoc{
				{Name: "window", Type: "webui-window! none!", Description: "Window to poll (none for all)", Optional: false},
			},
			Returns:  "[none!] When done",
			Examples: []string{"webui.poll window"},
	...
	webuiObj.Frame.Bind("close", value.NewFuncVal(value.NewNativeFunction(
		...
		func(args []core.Value, refValues map[string]core.Value, eval core.Evaluator) (core.Value, error) {
			return WebUIClose(args, refValues, eval, manager)
		},
		false,
		&NativeDoc{
			Category:    "WebUI",
	...
	webuiObj.Frame.Bind("ready?", value.NewFuncVal(value.NewNativeFunction(
		...
		func(args []core.Value, refValues map[string]core.Value, eval core.Evaluator) (core.Value, error) {
			return WebUIReady(args, refValues, eval, manager)
		},
		false,
		&NativeDoc{
			Category:    "WebUI",
			Summary:     "Check if window is ready",
			Description: `Returns true if the window has finished loading and is ready for interaction.`,
			Parameters: []ParamDoc{
				{Name: "window", Type: "webui-window!", Description: "Window to check", Optional: false},
			},
			Returns:  "[logic!] Ready status",
			Examples: []string{"webui.ready? window"},
			Tags:     []string{"webui", "ready", "status"},
		},
	)))
