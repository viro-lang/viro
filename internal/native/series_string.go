...
// validateUnicodeCodePoint validates that the given integer is a valid Unicode code point
func validateUnicodeCodePoint(codePoint int64) error {
	if codePoint < 0 || codePoint > 0x10FFFF {
		return verror.NewScriptError(verror.ErrIDInvalidOperation, [3]string{"code point out of range (0-0x10FFFF)", "", ""})
	}
	// Check for surrogate halves (0xD800–0xDFFF)
	if codePoint >= 0xD800 && codePoint <= 0xDFFF {
		return verror.NewScriptError(verror.ErrIDInvalidOperation, [3]string{"surrogate code points (0xD800-0xDFFF) not allowed", "", ""})
	}
	return nil
}

func CodepointsOf(args []core.Value, refValues map[string]core.Value, eval core.Evaluator) (core.Value, error) {
	if len(args) != 1 {
		return value.NewNoneVal(), arityError("codepoints-of", 1, len(args))
	}
	...
	runes := str.Runes()
	elements := make([]core.Value, len(runes))
	for i, r := range runes {
		elements[i] = value.NewIntVal(int64(r))
	}

	return value.NewBlockVal(elements), nil
}

func CodepointAt(args []core.Value, refValues map[string]core.Value, eval core.Evaluator) (core.Value, error) {
	if len(args) != 2 {
		return value.NewNoneVal(), arityError("codepoint-at", 2, len(args))
	}
	...
	if zeroBasedIndex < 0 || zeroBasedIndex >= len(runes) {
		...
		if defaultVal, hasDefault := refValues["default"]; hasDefault && defaultVal.GetType() != value.TypeNone {
			return defaultVal, nil
		}
		return value.NewNoneVal(), nil
	}

	return value.NewIntVal(int64(runes[zeroBasedIndex])), nil
}

func StringFromCodepoints(args []core.Value, refValues map[string]core.Value, eval core.Evaluator) (core.Value, error) {
	if len(args) != 1 {
		return value.NewNoneVal(), arityError("string-from-codepoints", 1, len(args))
	}
	...
	for i, elem := range elements {
		if elem.GetType() != value.TypeInteger {
			return value.NewNoneVal(), verror.NewScriptError(verror.ErrIDTypeMismatch, [3]string{"integer", value.TypeToString(elem.GetType()), ""})
		}

		codePoint64, _ := value.AsIntValue(elem)
		if err := validateUnicodeCodePoint(codePoint64); err != nil {
			return value.NewNoneVal(), err
		}

		runes[i] = rune(codePoint64)
	}

	return value.NewStringValue(string(runes)), nil
}
